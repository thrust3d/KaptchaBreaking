...
class ConvBN(tf.keras.layers.Layer):
  ...
  def __init__(self,
    ...
    if kernel_initializer == 'VarianceScaling':
      ...
      self._kernel_initializer = tf.compat.v1.keras.initializers.VarianceScaling(
  ...
  def fuse(self):
    if self.bn is not None and not self._use_separable_conv:
      ...
      w_conv_base = tf.transpose(a=conv_weights, perm=(3, 2, 0, 1))
      ...
      w_conv = tf.transpose(a=w_conv, perm=(2, 3, 1, 0))
...
class SAM(tf.keras.layers.Layer):
  ...
  def call(self, inputs, training=None):
    if self._use_pooling:
      depth_max = tf.reduce_max(input_tensor=inputs, axis=-1, keepdims=True)
      depth_avg = tf.reduce_mean(input_tensor=inputs, axis=-1, keepdims=True)
...
class CAM(tf.keras.layers.Layer):
  ...
  def call(self, inputs, training=None):
    depth_max = self._mlp(tf.reduce_max(input_tensor=inputs, axis=(1, 2)))
    depth_avg = self._mlp(tf.reduce_mean(input_tensor=inputs, axis=(1, 2)))

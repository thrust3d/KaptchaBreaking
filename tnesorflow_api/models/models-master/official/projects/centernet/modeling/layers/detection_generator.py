...
class CenterNetDetectionGenerator(tf.keras.layers.Layer):
  ...
  def process_heatmap(self,
    ...
    if not kernel_size or kernel_size == 1:
      ...
      feature_map_max_pool = tf.nn.max_pool2d(
          input=feature_map,
          ksize=kernel_size,
          strides=1,
          padding='SAME')
  ...
  def get_boxes(self,
    ...
    shape = tf.shape(input=height_width_predictions)
  ...
  def __call__(self, inputs):
    ...
    shape = tf.shape(input=ct_heatmaps)
    ...
    num_det = tf.reduce_sum(input_tensor=tf.cast(scores > 0, dtype=tf.int32), axis=1)

...
def cartesian_product(*tensors, repeat: int = 1) -> tf.Tensor:
  ...
  return tf.reshape(tf.transpose(a=tf.stack(tf.meshgrid(*tensors, indexing='ij')),
                                 perm=[*[i + 1 for i in range(len(tensors))], 0]),
                    (-1, len(tensors)))
...
def max_distance_for_overlap(height, width, min_iou):
  ...
  return tf.reduce_min(input_tensor=[distance_detection_offset,
...
def assign_center_targets(out_height: int,
  ...
  heatmap = tf.reduce_max(input_tensor=gaussian_per_box_per_class_map, axis=2)
...
def assign_centernet_targets(labels: Dict[str, tf.Tensor],
  ...
  if num_objects > 0:
    ...
    box_mask = tf.compat.v1.where(tf.range(max_num_instances) < num_objects, 1, 0)

...
class Encoder(object):
  ...
  def _get_word_reprs(self, pretrained_embeddings):
    with tf.compat.v1.variable_scope('word_embeddings'):
      word_embedding_matrix = tf.compat.v1.get_variable(
          'word_embedding_matrix', initializer=pretrained_embeddings)
      word_embeddings = tf.nn.embedding_lookup(
          params=word_embedding_matrix, ids=self._inputs.words)
      word_embeddings = tf.nn.dropout(word_embeddings, rate=1 - (self._inputs.keep_prob))
      word_embeddings *= tf.compat.v1.get_variable('emb_scale', initializer=1.0)

    if not self._config.use_chars:
      return word_embeddings

    with tf.compat.v1.variable_scope('char_embeddings'):
      char_embedding_matrix = tf.compat.v1.get_variable(
          'char_embeddings',
          shape=[embeddings.NUM_CHARS, self._config.char_embedding_size])
      char_embeddings = tf.nn.embedding_lookup(params=char_embedding_matrix,
                                               ids=self._inputs.chars)
      shape = tf.shape(input=char_embeddings)
      ...
      for filter_width in self._config.char_cnn_filter_widths:
        ...
        conv = tf.nn.dropout(tf.reduce_max(input_tensor=conv, axis=1),
                             rate=1 - (self._inputs.keep_prob))
  ...
  def _get_unidirectional_reprs(self, word_reprs):
    with tf.compat.v1.variable_scope('unidirectional_reprs'):
  ...
  def _get_bidirectional_reprs(self, uni_reprs):
    with tf.compat.v1.variable_scope('bidirectional_reprs'):

...
class Policy(object):
  def __init__(self, env_spec, internal_dim,
    ...
    self.matrix_init = tf.compat.v1.truncated_normal_initializer(stddev=0.01)
    self.vector_init = tf.compat.v1.constant_initializer(0.0)
  ...
  def get_cell(self):
    ...
    cell = tf.compat.v1.nn.rnn_cell.LSTMCell(self.cell_input_dim,
  ...
  def core(self, obs, prev_internal_state, prev_actions):
    ...
    batch_size = tf.shape(input=obs[0])[0]
    if not self.recurrent:
      prev_internal_state = tf.zeros([batch_size, self.rnn_state_dim])

    cell = self.get_cell()

    b = tf.compat.v1.get_variable('input_bias', [self.cell_input_dim],
    ...
    for i, (obs_dim, obs_type) in enumerate(self.env_spec.obs_dims_and_types):
      w = tf.compat.v1.get_variable('w_state%d' % i, [obs_dim, self.cell_input_dim],
    ...
    if self.input_prev_actions:
      if self.env_spec.combine_actions:  # TODO(ofir): clean this up
        ...
        for i, action_dim in enumerate(self.env_spec.orig_act_dims):
          ...
          w = tf.compat.v1.get_variable('w_prev_action%d' % i, [action_dim, self.cell_input_dim],
                              initializer=self.matrix_init)
          cell_input += tf.matmul(tf.one_hot(act, action_dim), w)
          prev_action = tf.cast(prev_action / action_dim, dtype=tf.int32)
      else:
        for i, (act_dim, act_type) in enumerate(self.env_spec.act_dims_and_types):
          w = tf.compat.v1.get_variable('w_prev_action%d' % i, [act_dim, self.cell_input_dim],
                              initializer=self.matrix_init)
  ...
  def sample_action(self, logits, sampling_dim,
    ...
    if self.env_spec.is_discrete(act_type):
      if greedy:
        ...
        act = tf.reshape(tf.random.categorical(logits=logits, num_samples=1), [-1])
    elif self.env_spec.is_box(act_type):
  ...
  def entropy(self, logits,
    ...
    if self.env_spec.is_discrete(act_type):
      ...
      std = logits[:, sampling_dim / 2:]
      entropy = tf.reduce_sum(
          input_tensor=0.5 * (1 + tf.math.log(2 * np.pi * tf.square(std))), axis=-1)
    else:
      assert False
  ...
  def self_kl(self, logits,
    ...
    if self.env_spec.is_discrete(act_type):
      ...
      self_kl = tf.reduce_sum(
          input_tensor=tf.math.log(std / my_std) +
  ...
  def log_prob_action(self, action, logits,
    ...
    if self.env_spec.is_discrete(act_type):
      ...
      act_log_prob = tf.reduce_sum(input_tensor=act_log_prob, axis=-1)
    else:
      assert False
  ...
  def sample_actions(self, output, actions=None, greedy=False):
    ...
    for i, (act_dim, act_type) in enumerate(self.env_spec.act_dims_and_types):
      ...
      if self.fixed_std and self.env_spec.is_box(act_type):
        ...
        log_std = tf.compat.v1.get_variable('std%d' % i, [1, sampling_dim // 2])
  ...
  def get_kl(self, my_logits, other_logits):
    ...
    for i, (act_dim, act_type) in enumerate(self.env_spec.act_dims_and_types):
      ...
      if self.env_spec.is_discrete(act_type):
        ...
        my_kl = tf.reduce_sum(input_tensor=my_probs * (my_log_probs - other_log_probs), axis=-1)
      elif self.env_spec.is_box(act_type):
        ...
        my_kl = tf.reduce_sum(
            input_tensor=tf.math.log(other_std / my_std) +
  ...
  def multi_step(self, all_obs, initial_state, all_actions):
    ...
    batch_size = tf.shape(input=initial_state)[0]
    time_length = tf.shape(input=all_obs[0])[0]
  ...
  def calculate_kl(self, my_logits, other_logits):
    ...
    batch_size = tf.shape(input=my_logits[0])[1]
    time_length = tf.shape(input=my_logits[0])[0]
...
class MLPPolicy(Policy):
  ...
  def get_cell(self):
    ...
    def mlp(cell_input, prev_internal_state):
      w1 = tf.compat.v1.get_variable('w1', [self.cell_input_dim, self.internal_dim])
      b1 = tf.compat.v1.get_variable('b1', [self.internal_dim])

      w2 = tf.compat.v1.get_variable('w2', [self.internal_dim, self.internal_dim])
      b2 = tf.compat.v1.get_variable('b2', [self.internal_dim])

      w3 = tf.compat.v1.get_variable('w3', [self.internal_dim, self.internal_dim])
      b3 = tf.compat.v1.get_variable('b3', [self.internal_dim])

      proj = tf.compat.v1.get_variable(

...
class Embedding(K.layers.Layer):
  ...
  def call(self, x):
    embedded = tf.nn.embedding_lookup(params=self.var, ids=x)
    if self.keep_prob < 1.:
      ...
      embedded = tf.nn.dropout(
          embedded, rate=1 - (self.keep_prob), noise_shape=(shape[0], 1, shape[2]))
    return embedded

  def _normalize(self, emb):
    weights = self.vocab_freqs / tf.reduce_sum(input_tensor=self.vocab_freqs)
...
class LSTM(object):
  ...
  def __call__(self, x, initial_state, seq_length):
    with tf.compat.v1.variable_scope(self.name, reuse=self.reuse) as vs:
  ...
  def call(self, inputs):
    ...
    if self.num_candidate_samples > -1:
      ...
      lm_loss = tf.nn.sampled_softmax_loss(
          weights=tf.transpose(a=self.lin_w),
          biases=self.lin_b,
    ...
    lm_loss = tf.identity(
        tf.reduce_sum(input_tensor=lm_loss * weights) / _num_labels(weights),
...
def classification_loss(logits, labels, weights):
  ...
  with tf.compat.v1.name_scope('classifier_loss'):
    # Logistic loss
    if inner_dim == 1:
    ...
    tf.compat.v1.summary.scalar('num_labels', num_lab)
    return tf.identity(
        tf.reduce_sum(input_tensor=weights * loss) / num_lab, name='classification_xentropy')
...
def accuracy(logits, targets, weights):
  ...
  with tf.compat.v1.name_scope('accuracy'):
    ...
    return tf.identity(
        tf.reduce_sum(input_tensor=weights * eq) / _num_labels(weights), name='accuracy')
...
def _num_labels(weights):
  ...
  num_labels = tf.reduce_sum(input_tensor=weights)
  num_labels = tf.compat.v1.where(tf.equal(num_labels, 0.), 1., num_labels)
  return num_labels
  ...
  with tf.compat.v1.name_scope('optimization'):
    ...
    if sync_replicas:
      ...
      if task_id == 0:  # Chief task
        ...
        tf.compat.v1.add_to_collection('chief_init_op', opt.get_init_tokens_op())
      else:
        local_init_op = opt.local_step_init_op
      tf.compat.v1.add_to_collection('local_init_op', local_init_op)
      tf.compat.v1.add_to_collection('ready_for_local_init_op',
...
def _summarize_vars_and_grads(grads_and_vars):
  ...
  for grad, var in grads_and_vars:
    ...
    tf.compat.v1.summary.scalar(tag('mean'), mean)
    with tf.compat.v1.name_scope(tag('stddev')):
      stddev = tf.sqrt(tf.reduce_mean(input_tensor=tf.square(var - mean)))
    tf.compat.v1.summary.scalar(tag('stddev'), stddev)
    tf.compat.v1.summary.scalar(tag('max'), tf.reduce_max(input_tensor=var))
    tf.compat.v1.summary.scalar(tag('min'), tf.reduce_min(input_tensor=var))
    tf.compat.v1.summary.histogram(tag('histogram'), var)
    ...
    if grad is not None:
      ...
      tf.compat.v1.summary.histogram(tag('gradient'), grad_values)
      tf.compat.v1.summary.scalar(tag('gradient_norm'), tf.linalg.global_norm([grad_values]))
    else:

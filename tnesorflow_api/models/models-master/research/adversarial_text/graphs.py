...
class VatxtModel(object):
  ...
  def classifier_graph(self):
    ...
    * Caches tensors: `cl_embedded`, `cl_logits`, `cl_loss`
    ...
    tf.compat.v1.summary.scalar('classification_loss', loss)
    ...
    tf.compat.v1.summary.scalar('adversarial_loss', adv_loss)

    total_loss = loss + adv_loss

    with tf.control_dependencies([inputs.save_state(next_state)]):
      total_loss = tf.identity(total_loss)
      tf.compat.v1.summary.scalar('total_classification_loss', total_loss)
    return total_loss
  ...
  def _lm_loss(self,
    ...
    if compute_loss:
      ...
      with tf.control_dependencies([inputs.save_state(next_state)]):
        ...
        tf.compat.v1.summary.scalar(loss_name, loss)
  ...
  def adversarial_loss(self):
    ...
    with tf.compat.v1.name_scope('adversarial_loss'):
...
def make_restore_average_vars_dict():
  ...
  for v in tf.compat.v1.global_variables():
    if v in tf.compat.v1.trainable_variables():
